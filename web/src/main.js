...
async function renderExhibitionParticipants(event){
  ...
  root.innerHTML=`<div class="entry-heading"><div><span class="tag">EXHIBITORS & WORKS</span><h2>${esc(event.exhibition_title||event.title)}｜出展者・作品管理</h2></div><button id="copyExhibitionCaptions" class="secondary" ${visibleWorks.length?'':'disabled'}>タイトル・キャプションを一括コピー</button></div><div class="summary-strip"><span>申込 ${entries.length}名</span><span>確定 ${submitted}名</span><span>作品 ${visibleWorks.length}点</span><span>QR登録 ${visibleWorks.filter(item=>item.work.instagram_qr_path).length}点</span></div><div id="exhibitorList" class="exhibitor-list"></div>`
