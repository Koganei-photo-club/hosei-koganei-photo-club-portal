...
async function renderEvent(id,context){
  ...
  try{
    ...
    view.innerHTML=`<section class="panel"><span class="tag">${eventLabel(event)}</span><h2>${esc(event.title)}</h2><dl><dt>日時</dt><dd>${fmt(event.starts_at)}${event.ends_at?` 〜 ${fmt(event.ends_at)}`:''}</dd><dt>場所</dt><dd>${esc(event.place)}</dd><dt>連絡先</dt><dd>${esc(event.contact)}</dd>${event.fee?`<dt>費用</dt><dd>${event.fee.toLocaleString()}円</dd>`:''}${event.payment_deadline?`<dt>支払期限</dt><dd>${fmt(event.payment_deadline)}</dd>`:''}</dl><p class="copy">${esc(event.details)}</p></section><section id="response" class="panel"></section>`
...
function renderEditor(event){
  ...
  form.camera_enabled.checked=Boolean(event?.camera_enabled);form.disposable_enabled.checked=Boolean(event?.disposable_enabled);form.shift_slots_text.value=(event?.shift_slots||[]).map(slot=>typeof slot==='string'?slot:slot.label).join('\n')
  const existingSlots=event?.shift_slots||[],conditions=()=>{const genre=form.genre.value,shooting=genre==='meeting'&&form.subtype.value==='shooting';document.querySelector('#subtypeField').classList.toggle('hidden',genre!=='meeting');document.querySelector('#shootingFields').classList.toggle('hidden',!shooting);document.querySelector('#campFields').classList.toggle('hidden',genre!=='camp');document.querySelector('#exhibitionFields').classList.toggle('hidden',genre!=='exhibition')}
  ...
  const save=async draft=>{try{
    ...
    if(!draft){if(!values.starts_at||!values.place.trim()||!values.contact.trim())throw new Error('保存には日時、場所、企画幹部の連絡先が必要です。');if(values.ends_at&&values.ends_at<values.starts_at)throw new Error('終了日時は開始日時以降にしてください。');if(values.genre==='camp'&&(!values.fee||!values.payment_deadline))throw new Error('合宿は費用と支払期限が必須です。');if(values.genre==='exhibition'&&(!values.exhibition_title.trim()||!values.max_works||!values.min_shift_people||!values.shift_slots_text.trim()))throw new Error('写真展の必須項目を入力してください。')}
