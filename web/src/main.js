  ...
  try{
    ...
    const save=async submitted=>{const saveButton=document.querySelector(submitted?'#submitEntry':'#saveEntryDraft'),editorList=[...editors.querySelectorAll('.work-editor')];try{
      ...
      for(const editor of editorList){
        ...
        const payload={title:editor.querySelector('[name=title]').value.trim(),caption:editor.querySelector('[name=caption]').value.trim(),note:editor.querySelector('[name=note]').value.trim(),original_image_path:originalPath,original_file_name:originalFileName,preview_image_path:previewPath,instagram_qr_path:qrPath,instagram_qr_file_name:qrFileName,status:submitted?'submitted':'draft'}
...
async function renderExhibitionParticipants(event){
  ...
  entries.forEach(entry=>{
    ...
    list.insertAdjacentHTML('beforeend',`<article class="exhibitor-card"><div class="exhibitor-head"><div><span class="tag">${entry.status==='submitted'?'申込済み':entry.status==='withdrawn'?'取り下げ':'下書き'}</span><h3>${esc(member.name||'部員情報なし')}</h3><p>${esc(member.member_no||'')} ${esc(affiliation)}</p></div><span class="status">${works.length}作品</span></div>${entry.note?`<p class="muted">出展備考：${esc(entry.note)}</p>`:''}<div class="admin-work-list">${works.length?works.map(work=>`<section class="admin-work-card"><div class="admin-work-image">${work.preview_image_path?`<span class="storage-image" data-storage-path="${esc(work.preview_image_path)}" data-alt="${esc(work.title||'作品プレビュー')}">プレビュー読込中…</span>`:'<span class="muted">プレビューなし</span>'}</div><div class="admin-work-copy"><div class="work-meta"><span class="tag">${work.display_no?`No.${esc(work.display_no)}`:`WORK ${work.sort_order}`}</span><span>${esc(exhibitionWorkStatus(work.status))}</span></div><h3>${esc(work.title||'作品名未入力')}</h3><p class="caption-text">${work.caption?esc(work.caption):'<span class="muted">キャプション未入力</span>'}</p><p class="muted">原画像：${esc(work.original_file_name||work.original_image_path?.split('/').pop()||'未登録')}</p>${work.note?`<p class="muted">作品備考：${esc(work.note)}</p>`:''}</div><div class="admin-work-qr"><strong>Instagram QR</strong>${work.instagram_qr_path?`<span class="storage-image qr-image" data-storage-path="${esc(work.instagram_qr_path)}" data-alt="${esc(`${work.title||'作品'}のInstagram QRコード`)}">QR読込中…</span><small>${esc(work.instagram_qr_file_name||'登録済み')}</small>`:'<span class="muted">未登録</span>'}</div></section>`).join(''):'<p class="muted">作品はまだ登録されていません。</p>'}</div></article>`)
  })
  root.querySelector('#copyExhibitionCaptions').onclick=async()=>{const text=visibleWorks.map(({entry,work},index)=>{const member=entry.members||{};return[`【作品${index+1}${work.display_no?`／No.${work.display_no}`:''}】`,`出展者：${member.name||''}（${member.member_no||''}）`,`作品名：${work.title||''}`,`キャプション：${work.caption||''}`,`Instagram QR：${work.instagram_qr_path?'あり':'なし'}`].join('\n')}).join('\n\n');try{await copyText(text);message('全作品のタイトルとキャプションをコピーしました。')}catch(copyError){failure('クリップボードへコピーできませんでした。') }}
