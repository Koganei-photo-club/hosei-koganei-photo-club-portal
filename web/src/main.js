...
const managedOriginalFileName=(member,work)=>{const ext=work.original_image_path?.split('.').pop()||'jpg';return `${safeStorageFileName(member.name,member.member_no)}_作品${work.sort_order}.${ext}`}
  ...
  try{
    ...
    const save=async submitted=>{const saveButton=document.querySelector(submitted?'#submitEntry':'#saveEntryDraft'),editorList=[...editors.querySelectorAll('.work-editor')];try{
      ...
      for(const editor of editorList){
        ...
        if(file){
          const prefix=`${event.id}/${context.member.id}/${workId}`,internalMemberNo=context.member.member_no.replace(/[^A-Za-z0-9_-]/g,'_'),newOriginalPath=`${prefix}/${internalMemberNo}_work-${sortOrder}.${originalExtension(file)}`
