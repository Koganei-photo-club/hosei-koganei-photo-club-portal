...
async function navigate(){
  try{
    if(!overdueChecked){overdueChecked=true;const {error}=await supabase.rpc('apply_overdue_payment_cancellations');if(error)console.warn('期限超過処理を実行できませんでした。',error)}
...
async function renderPortal(context){
  ...
  try{
    ...
    if(context.member){
      const {data:entries,error:entryError}=await supabase.from('exhibition_entries').select('event_id,status,exhibition_works(status,orientation,print_size)').eq('member_id',context.member.id)
    ...
    events?.forEach(event=>{const response=event.event_responses?.[0],entry=exhibitionEntries[event.id],entryWorks=(entry?.exhibition_works||[]).filter(work=>work.status!=='withdrawn'),state=entryWorks.some(work=>work.status==='rejected'||!work.orientation||!work.print_size)?'要修正の作品があります':entryWorks.length&&entryWorks.every(work=>work.status==='accepted')?'全作品を確認済み':entry?.status==='submitted'?'出展申込済み':entry?.status==='draft'?'出展申込を下書き保存中':entry?.status==='withdrawn'?'出展申込を取り下げ済み':'';eventRoot.insertAdjacentHTML('beforeend',`<a class="card" href="#/event/${event.id}"><div><span class="tag">${eventLabel(event)}</span><h3>${esc(event.title)}</h3><p>${fmt(event.starts_at)}・${esc(event.place)}</p>${event.genre==='exhibition'&&state?`<p class="status">${state}</p>`:response?`<p class="status">${response.cancelled_at?'キャンセル済み':`回答済み：${esc(response.attendance)}`}</p>`:''}</div><strong>→</strong></a>`)})
...
const safeStorageFileName=(value,fallback)=>value.replace(/[\\/:*?"<>|\u0000-\u001f]/g,'_').trim()||fallback
const orientationLabel=value=>value==='portrait'?'縦':value==='landscape'?'横':'未選択'
const printSizeLabel=(size,detail='')=>size==='composite'?`組み写真${detail?`（${detail}）`:''}`:size==='other'?`その他${detail?`（${detail}）`:''}`:size||'未選択'
...
async function renderExhibitionEvent(event,context){
  ...
  try{
    ...
    if(error)throw error
    hideMessage();const view=document.querySelector('#view'),allWorks=(entry?.exhibition_works||[]).sort((a,b)=>a.sort_order-b.sort_order),works=allWorks.filter(work=>work.status!=='withdrawn'),hasRejected=works.some(work=>work.status==='rejected'||!work.orientation||!work.print_size),allAccepted=works.length>0&&works.every(work=>work.status==='accepted'&&work.orientation&&work.print_size),entryState=hasRejected?'要修正':allAccepted?'確認済み':entry?entry.status==='submitted'?'申込済み':entry.status==='withdrawn'?'取り下げ済み':'下書き':'未入力'
    ...
    const addEditor=(work=null)=>{
      ...
      const locked=work?.status==='accepted'&&work?.orientation&&work?.print_size,storedFileName=work?.original_file_name||work?.original_image_path?.split('/').pop()||'',storedQrName=work?.instagram_qr_file_name||work?.instagram_qr_path?.split('/').pop()||'',fileInput=`<input type="file" name="original" class="${work?.original_image_path?'hidden':''}" accept="image/jpeg,image/png,image/tiff,image/heic,image/heif,.jpg,.jpeg,.png,.tif,.tiff,.heic,.heif" ${locked?'disabled':''}>`,fileControl=work?.original_image_path?`<div class="registered-file"><span>登録済み：<strong>${esc(storedFileName)}</strong></span>${locked?'':'<button type="button" class="secondary replace-image">画像を差し替える</button>'}</div>${fileInput}`:fileInput,qrInput=`<input type="file" name="instagram_qr" class="${work?.instagram_qr_path?'hidden':''}" accept="image/jpeg,image/png,image/webp,.jpg,.jpeg,.png,.webp" ${locked?'disabled':''}>`,qrControl=work?.instagram_qr_path?`<div class="registered-file"><span>登録済み：<strong>${esc(storedQrName)}</strong></span>${locked?'':'<button type="button" class="secondary replace-qr">QRコードを差し替える</button>'}</div>${qrInput}`:qrInput
      ...
      editor.querySelector('.replace-image')?.addEventListener('click',click=>{click.currentTarget.classList.add('hidden');editor.querySelector('[name=original]').classList.remove('hidden')})
      editor.querySelector('.replace-qr')?.addEventListener('click',click=>{click.currentTarget.classList.add('hidden');editor.querySelector('[name=instagram_qr]').classList.remove('hidden')})
      ...
      editor.querySelector('[name=instagram_qr]').onchange=change=>{const file=change.target.files[0];if(file&&!allowedQrTypes.has(file.type)){change.target.value='';failure('QRコードはJPEG・PNG・WebPを選択してください。')}}
      const sizeSelect=editor.querySelector('[name=print_size]'),sizeDetail=editor.querySelector('.print-size-detail');sizeSelect.onchange=()=>sizeDetail.classList.toggle('hidden',!['composite','other'].includes(sizeSelect.value))
      editor.querySelector('.remove-work').onclick=async()=>{
        if(!work){editor.remove();renumber();updateEntryButtons();return}
        ...
        try{
          ...
          const paths=[work.original_image_path,work.preview_image_path,work.instagram_qr_path].filter(Boolean)
          for(const [bucket,path] of [['exhibition-originals',work.original_image_path],['exhibition-previews',work.preview_image_path],['exhibition-previews',work.instagram_qr_path]])if(path){const {error:removeError}=await supabase.storage.from(bucket).remove([path]);if(removeError)throw removeError}
    ...
    works.forEach(addEditor);if(!works.length)addEditor()
    const draftButton=document.querySelector('#saveEntryDraft'),submitButton=document.querySelector('#submitEntry'),formSnapshot=()=>JSON.stringify({entryNote:form.entry_note.value,works:[...editors.querySelectorAll('.work-editor')].map(editor=>{const original=editor.querySelector('[name=original]').files[0],qr=editor.querySelector('[name=instagram_qr]').files[0];return{id:editor.dataset.id,title:editor.querySelector('[name=title]').value,orientation:editor.querySelector('[name=orientation]').value,printSize:editor.querySelector('[name=print_size]').value,printSizeDetail:editor.querySelector('[name=print_size_detail]').value,artistName:editor.querySelector('[name=artist_name]').value,cameraName:editor.querySelector('[name=camera_name]').value,lensOther:editor.querySelector('[name=lens_other]').value,description:editor.querySelector('[name=description]').value,note:editor.querySelector('[name=note]').value,original:original?`${original.name}:${original.size}:${original.lastModified}`:'',qr:qr?`${qr.name}:${qr.size}:${qr.lastModified}`:''}})}),initialSnapshot=formSnapshot(),updateEntryButtons=()=>{const changed=formSnapshot()!==initialSnapshot;draftButton.disabled=!changed;submitButton.disabled=entry?.status==='submitted'&&!changed;submitButton.textContent=entry?.status==='submitted'?(changed?(hasRejected?'修正内容を再提出':'変更内容で再確定'):hasRejected?'修正してください':'申込済み'):'出展申込を確定'}
    updateEntryButtons();form.addEventListener('input',updateEntryButtons);form.addEventListener('change',updateEntryButtons);addButton.onclick=()=>{addEditor();updateEntryButtons()}
    const save=async submitted=>{const saveButton=document.querySelector(submitted?'#submitEntry':'#saveEntryDraft'),editorList=[...editors.querySelectorAll('.work-editor')];try{
      ...
      editorList.forEach((editor,index)=>{if(editor.dataset.locked==='true')return;const title=editor.querySelector('[name=title]').value.trim(),orientation=editor.querySelector('[name=orientation]').value,printSize=editor.querySelector('[name=print_size]').value,printSizeDetail=editor.querySelector('[name=print_size_detail]').value.trim(),artistName=editor.querySelector('[name=artist_name]').value.trim(),cameraName=editor.querySelector('[name=camera_name]').value.trim(),file=editor.querySelector('[name=original]').files[0],qrFile=editor.querySelector('[name=instagram_qr]').files[0],hasOriginal=Boolean(editor.dataset.originalPath);if(submitted&&!title)throw new Error(`作品${index+1}の作品名を入力してください。`);if(submitted&&!orientation)throw new Error(`作品${index+1}の向きを選択してください。`);if(submitted&&!printSize)throw new Error(`作品${index+1}の出展サイズを選択してください。`);if(submitted&&['composite','other'].includes(printSize)&&!printSizeDetail)throw new Error(`作品${index+1}のサイズ詳細を入力してください。`);if(submitted&&!artistName)throw new Error(`作品${index+1}の作者名・ペンネームを入力してください。`);if(submitted&&!cameraName)throw new Error(`作品${index+1}のCameraを入力してください。`);if(submitted&&!file&&!hasOriginal)throw new Error(`作品${index+1}の原画像を選択してください。`);if(file&&file.size>52428800)throw new Error(`作品${index+1}の原画像が50MBを超えています。`);if(file&&!allowedOriginalTypes.has(file.type))throw new Error(`作品${index+1}の画像形式に対応していません。`);if(qrFile&&qrFile.size>10485760)throw new Error(`作品${index+1}のQRコードが10MBを超えています。`);if(qrFile&&!allowedQrTypes.has(qrFile.type))throw new Error(`作品${index+1}のQRコード形式に対応していません。`)})
      ...
      for(const editor of editorList){
        ...
        let workId=editor.dataset.id,sortOrder=Number(editor.dataset.sortOrder)||nextSort++,originalPath=editor.dataset.originalPath||null,originalFileName=editor.dataset.originalFileName||'',previewPath=editor.dataset.previewPath||null,qrPath=editor.dataset.qrPath||null,qrFileName=editor.dataset.qrFileName||''
        ...
        if(file){
          const prefix=`${event.id}/${context.member.id}/${workId}`,internalName=safeStorageFileName(context.member.name,context.member.member_no),newOriginalPath=`${prefix}/${internalName}_作品${sortOrder}.${originalExtension(file)}`
        ...
        if(qrFile){
          const prefix=`${event.id}/${context.member.id}/${workId}`,newQrPath=`${prefix}/instagram-qr.${qrExtension(qrFile)}`
          const {error:qrUploadError}=await supabase.storage.from('exhibition-previews').upload(newQrPath,qrFile,{upsert:true,contentType:qrFile.type});if(qrUploadError)throw qrUploadError
          if(qrPath&&qrPath!==newQrPath){const {error:oldQrError}=await supabase.storage.from('exhibition-previews').remove([qrPath]);if(oldQrError)console.warn('以前のQRコードを削除できませんでした。',oldQrError)}
          qrPath=newQrPath;qrFileName=qrFile.name
        }
        const payload={title:editor.querySelector('[name=title]').value.trim(),orientation:editor.querySelector('[name=orientation]').value,print_size:editor.querySelector('[name=print_size]').value,print_size_detail:editor.querySelector('[name=print_size_detail]').value.trim(),artist_name:editor.querySelector('[name=artist_name]').value.trim(),camera_name:editor.querySelector('[name=camera_name]').value.trim(),lens_other:editor.querySelector('[name=lens_other]').value.trim(),description:editor.querySelector('[name=description]').value.trim(),note:editor.querySelector('[name=note]').value.trim(),original_image_path:originalPath,original_file_name:originalFileName,preview_image_path:previewPath,instagram_qr_path:qrPath,instagram_qr_file_name:qrFileName,status:submitted?'submitted':'draft'}
...
async function renderAdmin(context){
  ...
  try{
    ...
    list.querySelectorAll('.admin-row').forEach(row=>{const event=events.find(e=>e.id===row.dataset.id);row.querySelector('.participants').onclick=()=>renderParticipants(event);row.querySelector('.edit').onclick=()=>renderEditor(event);row.querySelector('.publish').onclick=async()=>{if(!confirm(`「${event.title}」の公開状態を変更しますか？`))return;await supabase.from('events').update({published:!event.published,updated_at:new Date().toISOString()}).eq('id',event.id);renderAdmin()};row.querySelector('.delete').onclick=async()=>{if(!confirm(`「${event.title}」を削除しますか？\n回答記録は保持されます。`))return;const {error}=await supabase.from('events').update({deleted_at:new Date().toISOString(),published:false}).eq('id',event.id);if(error)failure(error);else renderAdmin()}})
...
async function renderParticipants(event){
  ...
  const root=document.querySelector('#participantAdmin');document.querySelector('#editor').classList.add('hidden');root.classList.remove('hidden');root.innerHTML='<p>参加者情報を読み込んでいます…</p>';root.scrollIntoView({behavior:'smooth'})
  const {data:responses,error}=await supabase.from('event_responses').select('*,members(member_no,name,email,grade,faculty,department,graduate_school,major)').eq('event_id',event.id).order('submitted_at')
  if(error){failure(error);root.classList.add('hidden');return}
  const joined=responses.filter(response=>response.attendance==='参加'&&!response.cancelled_at).length,paid=responses.filter(response=>response.payment_status==='paid').length,unpaid=responses.filter(response=>response.payment_status==='unpaid').length
  ...
  const list=root.querySelector('#participantList');if(!responses.length){list.innerHTML='<p class="muted">回答はまだありません。</p>';return}
  responses.forEach(response=>{
    const member=response.members||{},paymentTarget=event.fee_enabled&&event.fee>0&&response.attendance==='参加',affiliation=[member.grade,member.faculty||member.graduate_school,member.department||member.major].filter(Boolean).join('・')
  ...
  list.querySelectorAll('.payment-status').forEach(select=>{const previous=select.value;select.onchange=async()=>{const row=select.closest('.participant-row'),next=select.value,label=paymentLabel(next);if(!confirm(`支払い状況を「${label}」へ変更しますか？`)){select.value=previous;return}select.disabled=true;const {error}=await supabase.rpc('set_event_payment_status',{p_response_id:row.dataset.id,p_status:next});if(error){select.value=previous;select.disabled=false;failure(error);return}message(`支払い状況を「${label}」へ変更しました。`);renderParticipants(event)}})
}
...
async function downloadStorageFile(bucket,path,fileName){
  const {data,error}=await supabase.storage.from(bucket).download(path);if(error)throw error
  const url=URL.createObjectURL(data),link=document.createElement('a');link.href=url;link.download=fileName||path.split('/').pop()||'download';document.body.appendChild(link);link.click();link.remove();setTimeout(()=>URL.revokeObjectURL(url),1000)
}

async function renderExhibitionParticipants(event){
  const root=document.querySelector('#participantAdmin');document.querySelector('#editor').classList.add('hidden');root.classList.remove('hidden');root.innerHTML='<p>出展者と作品情報を読み込んでいます…</p>';root.scrollIntoView({behavior:'smooth'})
  const {data:entries,error}=await supabase.from('exhibition_entries').select('*,members(member_no,name,grade,faculty,department,graduate_school,major),exhibition_works(*)').eq('event_id',event.id).order('created_at')
  if(error){failure(error);root.classList.add('hidden');return}
  const visibleWorks=(entries||[]).flatMap(entry=>(entry.exhibition_works||[]).filter(work=>work.status!=='withdrawn').map(work=>({entry,work}))),submitted=entries.filter(entry=>entry.status==='submitted').length
  ...
  const list=root.querySelector('#exhibitorList');if(!entries.length){list.innerHTML='<p class="muted">出展申込はまだありません。</p>';return}
  entries.forEach(entry=>{
    const member=entry.members||{},affiliation=[member.grade,member.faculty||member.graduate_school,member.department||member.major].filter(Boolean).join('・'),works=(entry.exhibition_works||[]).filter(work=>work.status!=='withdrawn').sort((a,b)=>(a.sort_order||0)-(b.sort_order||0))
    list.insertAdjacentHTML('beforeend',`<article class="exhibitor-card"><div class="exhibitor-head"><div><span class="tag">${entry.status==='submitted'?'申込済み':entry.status==='withdrawn'?'取り下げ':'下書き'}</span><h3>${esc(member.name||'部員情報なし')}</h3><p>${esc(member.member_no||'')} ${esc(affiliation)}</p></div><span class="status">${works.length}作品</span></div>${entry.note?`<p class="muted">出展備考：${esc(entry.note)}</p>`:''}<div class="admin-work-list">${works.length?works.map(work=>`<section class="admin-work-card" data-work-id="${work.id}"><div class="admin-work-image">${work.preview_image_path?`<span class="storage-image" data-storage-path="${esc(work.preview_image_path)}" data-alt="${esc(work.title||'作品プレビュー')}">プレビュー読込中…</span>`:'<span class="muted">プレビューなし</span>'}${work.original_image_path?`<button type="button" class="secondary download-original" data-original-path="${esc(work.original_image_path)}" data-file-name="${esc(work.original_image_path.split('/').pop()||'original-image')}">原画像をダウンロード</button>`:''}</div><div class="admin-work-copy"><div class="work-meta"><span class="tag">${work.display_no?`No.${esc(work.display_no)}`:`WORK ${work.sort_order}`}</span><span>${esc(exhibitionWorkStatus(work.status))}</span></div><h3>${esc(work.title||'作品名未入力')}</h3><dl class="caption-details"><dt>向き</dt><dd>${esc(orientationLabel(work.orientation))}</dd><dt>出展サイズ</dt><dd>${esc(printSizeLabel(work.print_size,work.print_size_detail))}</dd><dt>作者</dt><dd>${work.artist_name?esc(work.artist_name):'<span class="muted">未入力</span>'}</dd><dt>Camera</dt><dd>${work.camera_name?esc(work.camera_name):'<span class="muted">未入力</span>'}</dd><dt>Lens, other</dt><dd>${work.lens_other?esc(work.lens_other):'<span class="muted">未入力</span>'}</dd><dt>Description</dt><dd class="caption-text">${work.description?esc(work.description):'<span class="muted">未入力</span>'}</dd></dl><p class="muted">アップロード元：${esc(work.original_file_name||'不明')}</p><p class="muted">管理ファイル名：${esc(work.original_image_path?.split('/').pop()||'未登録')}</p>${work.note?`<p class="muted">作品備考：${esc(work.note)}</p>`:''}<div class="work-admin-controls"><label>作品番号<input class="display-no" value="${esc(work.display_no||'')}" placeholder="例：01"></label><label>確認状態<select class="review-status"><option value="submitted" ${work.status==='submitted'||work.status==='draft'?'selected':''}>提出済み</option><option value="accepted" ${work.status==='accepted'?'selected':''}>確認済み</option><option value="rejected" ${work.status==='rejected'?'selected':''}>要修正</option></select></label><button type="button" class="update-work">作品情報を更新</button></div></div><div class="admin-work-qr"><strong>Instagram QR</strong>${work.instagram_qr_path?`<span class="storage-image qr-image" data-storage-path="${esc(work.instagram_qr_path)}" data-alt="${esc(`${work.title||'作品'}のInstagram QRコード`)}">QR読込中…</span><small>${esc(work.instagram_qr_file_name||'登録済み')}</small><button type="button" class="secondary download-qr" data-qr-path="${esc(work.instagram_qr_path)}" data-file-name="${esc(work.instagram_qr_file_name||'instagram-qr')}">QR画像をダウンロード</button>`:'<span class="muted">未登録</span>'}</div></section>`).join(''):'<p class="muted">作品はまだ登録されていません。</p>'}</div></article>`)
  })
  root.querySelector('#copyExhibitionCaptions').onclick=async()=>{const text=visibleWorks.map(({entry,work},index)=>{const member=entry.members||{};return[`【作品${index+1}${work.display_no?`／No.${work.display_no}`:''}】`,`出展者：${member.name||''}（${member.member_no||''}）`,`作品名：${work.title||''}`,`向き：${orientationLabel(work.orientation)}`,`出展サイズ：${printSizeLabel(work.print_size,work.print_size_detail)}`,`作者：${work.artist_name||''}`,`Camera：${work.camera_name||''}`,`Lens, other：${work.lens_other||''}`,`Description：${work.description||''}`,`Instagram QR：${work.instagram_qr_path?'あり':'なし'}`].join('\n')}).join('\n\n');try{await copyText(text);message('全作品のタイトルとキャプションをコピーしました。')}catch(copyError){failure('クリップボードへコピーできませんでした。') }}
  root.querySelector('#assignDisplayNumbers').onclick=async()=>{if(!confirm('未採番の作品へ01から順に作品番号を付けますか？\nすでに採番済みの作品は変更しません。'))return;const button=root.querySelector('#assignDisplayNumbers');button.disabled=true;const {data,error:assignError}=await supabase.rpc('admin_assign_exhibition_display_numbers',{p_event_id:event.id,p_start:1,p_padding:2});if(assignError){button.disabled=false;failure(assignError);return}await renderExhibitionParticipants(event);message(`${data.assignedCount}作品へ番号を付けました。`)}
  root.querySelectorAll('.storage-image').forEach(async target=>{const {data,error:imageError}=await supabase.storage.from('exhibition-previews').createSignedUrl(target.dataset.storagePath,900);if(imageError){target.textContent='画像を表示できませんでした。';return}target.innerHTML=`<img src="${esc(data.signedUrl)}" alt="${esc(target.dataset.alt)}">`})
  root.querySelectorAll('.download-qr').forEach(button=>button.onclick=async()=>{button.disabled=true;try{await downloadStorageFile('exhibition-previews',button.dataset.qrPath,button.dataset.fileName);message('QR画像をダウンロードしました。')}catch(downloadError){failure(downloadError)}finally{button.disabled=false}})
  root.querySelectorAll('.download-original').forEach(button=>button.onclick=async()=>{button.disabled=true;try{await downloadStorageFile('exhibition-originals',button.dataset.originalPath,button.dataset.fileName);message('原画像をダウンロードしました。')}catch(downloadError){failure(downloadError)}finally{button.disabled=false}})
  root.querySelectorAll('.update-work').forEach(button=>button.onclick=async()=>{const card=button.closest('.admin-work-card'),displayNo=card.querySelector('.display-no').value.trim(),status=card.querySelector('.review-status').value;if(!confirm(`作品番号を「${displayNo||'未採番'}」、確認状態を「${exhibitionWorkStatus(status)}」へ更新しますか？`))return;button.disabled=true;const {error:updateError}=await supabase.rpc('admin_update_exhibition_work',{p_work_id:card.dataset.workId,p_display_no:displayNo,p_status:status});if(updateError){button.disabled=false;failure(updateError);return}await renderExhibitionParticipants(event);message('作品番号と確認状態を更新しました。')})
}
...
function renderEditor(event){
  ...
  const form=document.querySelector('#eventForm'),local=value=>value?new Date(new Date(value)-new Date(value).getTimezoneOffset()*60000).toISOString().slice(0,16):'',asIso=value=>value?new Date(value).toISOString():null
  ...
  const save=async draft=>{try{
    ...
    const feeCapable=values.genre==='camp'||(values.genre==='meeting'&&values.subtype==='dining'),feeEnabled=feeCapable&&form.fee_enabled.checked,deadlineEnabled=feeEnabled&&form.payment_deadline_enabled.checked,payload={title:values.title.trim(),genre:values.genre,subtype:values.genre==='meeting'?values.subtype:'',starts_at:asIso(values.starts_at),ends_at:asIso(values.ends_at),place:values.place.trim(),contact:values.contact.trim(),details:values.details.trim(),fee_enabled:feeEnabled,fee:feeEnabled?Number(values.fee||0):0,payment_deadline_enabled:deadlineEnabled,payment_deadline:deadlineEnabled?asIso(values.payment_deadline):null,exhibition_title:values.genre==='exhibition'?values.exhibition_title.trim():'',max_works:values.genre==='exhibition'?Number(values.max_works||0):0,min_shift_people:values.genre==='exhibition'?Number(values.min_shift_people||0):0,shift_slots:values.genre==='exhibition'?shift_slots:[],camera_enabled:values.genre==='meeting'&&values.subtype==='shooting'&&form.camera_enabled.checked,disposable_enabled:values.genre==='meeting'&&values.subtype==='shooting'&&form.disposable_enabled.checked,status:draft?'draft':'saved',updated_at:new Date().toISOString(),updated_by:session.user.email}
