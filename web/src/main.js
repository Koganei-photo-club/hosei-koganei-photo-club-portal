...
async function renderPortal(context){
  ...
  try{
    ...
    if(context.member){
      const {data:entries,error:entryError}=await supabase.from('exhibition_entries').select('event_id,status,exhibition_works(status)').eq('member_id',context.member.id)
...
async function renderExhibitionEvent(event,context){
  ...
  try{
    ...
    if(error)throw error
    hideMessage();const view=document.querySelector('#view'),allWorks=(entry?.exhibition_works||[]).sort((a,b)=>a.sort_order-b.sort_order),works=allWorks.filter(work=>work.status!=='withdrawn'),hasRejected=works.some(work=>work.status==='rejected'),allAccepted=works.length>0&&works.every(work=>work.status==='accepted'),entryState=hasRejected?'要修正':allAccepted?'確認済み':entry?entry.status==='submitted'?'申込済み':entry.status==='withdrawn'?'取り下げ済み':'下書き':'未入力'
